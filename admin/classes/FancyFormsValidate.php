...
class FancyFormsValidate {
    ...
    public static function validate($values) {
        ...
        if (!isset($values['form_id']) || !isset($values['item_meta'])) {
            $errors['form'] = esc_html__('There was a problem with your submission. Please try again.', 'fancy-forms');
            return $errors;
        }

        if (FancyFormsHelper::is_admin_page() && is_user_logged_in() && (!isset($values['fancyforms_submit_entry_' . $values['form_id']]) || !wp_verify_nonce($values['fancyforms_submit_entry_' . $values['form_id']], 'fancyforms_submit_entry_nonce') )) {
            $errors['form'] = esc_html__('Nounce Error', 'fancy-forms');
        }

        $fields = FancyFormsFields::get_form_fields($values['form_id']);
    ...
    public static function validate_field($field, &$errors, $values) {
        ...
        if (!is_array($value)) {
            ...
            if ($field->required == '1' && empty($value)) {
                $errors['field' . $field_id] = FancyFormsFields::get_error_msg($field, 'blank');
            }
        }
    ...
    public static function sanitize_entries($values) {
        $sanitize_method = array(
            'fancyforms_action' => 'sanitize_title',
            'form_id' => 'absint',
