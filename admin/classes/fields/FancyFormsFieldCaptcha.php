...
class FancyFormsFieldCaptcha extends FancyFormsFieldType {
    ...
    protected function load_field_scripts() {
        ...
        wp_enqueue_script('captcha-api', $api_js_url, array(), FANCYFORMS_VERSION, true);
    }

    protected function api_url() {
        $fancyforms_settings = FancyFormsSettings::get_settings();
        return $this->recaptcha_api_url($fancyforms_settings);
    }
    ...
    protected function validate_against_api($args) {
        ...
        if (is_wp_error($resp)) {
            ...
            $errors['field' . $args['id']] = esc_html__('There was a problem verifying your captcha', 'fancy-forms');
        ...
        if (isset($response['success']) && !$response['success']) {
            $invalid_message = FancyFormsFields::get_option($this->field, 'invalid');
            if ($invalid_message === esc_html__('The reCAPTCHA was not entered correctly', 'fancy-forms')) {
                $invalid_message = '';
            }
    ...
    public function validate($args) {
        $errors = array();
        if (!self::should_show_captcha()) {
            $errors['field' . $args['id']] = esc_html__('The reCAPTCHA keys are not entered.', 'fancy-forms');
            return $errors;
        } else {
    ...
    protected function send_api_check($args) {
        ...
        $arg_array = array(
            'body' => array(
                ...
                'response' => $args['value'],
                'remoteip' => FancyFormsHelper::get_ip_address(),
    ...
    protected function input_html() {
        $html = '';
        if (is_admin()) {
            if (!FancyFormsFieldCaptcha::should_show_captcha()) {
                ?>
                <div class="howto">
                    <?php esc_html_e('This field is not set up yet.', 'fancy-forms'); ?>
                </div>
                <?php
            } else {
                $image_name = FancyFormsFieldCaptcha::get_captcha_image_name();
                ?>
                <img src="<?php echo esc_url(FANCYFORMS_URL . '/img/' . $image_name . '.png'); ?>" style="width: 304px;" />
