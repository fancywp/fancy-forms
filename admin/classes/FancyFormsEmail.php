...
class FancyFormsEmail {
    ...
    public function send_email() {
        ...
        $entry = FancyFormsEntry::get_entry_vars($this->entry_id);
        $metas = $entry->metas;
        ...
        $file_img_placeholder = FANCYFORMS_URL . '/img/attachment.png';
        ...
        foreach ($metas as $item => $value) {
            ...
            $entry_value = FancyFormsHelper::unserialize_or_decode($value['value']);
            $entry_type = FancyFormsHelper::unserialize_or_decode($value['type']);
            ...
            if ($entry_type == 'upload' && trim($entry_value)) {
                ...
                foreach ($files_arr as $file) {
                    ...
                    $attachments[] = $upload_dir['basedir'] . FANCYFORMS_UPLOAD_DIR . '/' . $file_name;
        ...
        do_action('fancyforms_after_email', array(
        ...
        if ($mail) {
            if (isset($form_settings['enable_ar']) && $form_settings['enable_ar'] == 'on') {
                ...
                $from_ar_name = isset($form_settings['from_ar_name']) && ($form_settings['from_ar_name'] != '') ? esc_html($form_settings['from_ar_name']) : esc_html__('No Name', 'fancy-forms');
                $email_subject = isset($form_settings['email_subject_ar']) && ($form_settings['email_subject_ar'] != '') ? esc_html($form_settings['email_subject_ar']) : esc_html__('New Form Submission', 'fancy-forms');
                $email_message = wpautop(isset($form_settings['email_message_ar']) ? esc_html($form_settings['email_message_ar']) : '');
                $settings = FancyFormsSettings::get_settings();
                $header_image = $settings['header_image'];

                ob_start();
                include(FANCYFORMS_PATH . 'admin/settings/email-templates/template1.php');
    ...
    public function get_entry_rows($email_template) {
        ...
        foreach ($entry->metas as $id => $value) {
            ...
            $entry_rows .= call_user_func('FancyFormsEmail::' . $email_template, $title, $entry_value, $count);
        }
        return $entry_rows;
    }
