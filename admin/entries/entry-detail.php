...
<div class="fancyforms-form-entry-details-wrap wrap">
    <h1></h1>
    <div id="fancyforms-form-entry-details">
        <div class="fancyforms-page-title">
            <h3>
                <span><?php esc_html_e('Entry', 'fancy-forms'); ?></span>
                <span class="fancyforms-sub-label">
                    <?php printf(esc_html__('(ID %d)', 'fancy-forms'), absint($entry->id)); ?>
                </span>
            </h3>
        </div>
        <table>
                ...
                foreach ($entry->metas as $id => $value) {
                    ...
                    if ($entry_type == 'upload' && $entry_value) {
                        ...
                        foreach ($files_arr as $file) {
                            ...
                            $upload_value .= '<div class="fancyforms-form-entry-preview">';
                            $upload_value .= '<div class="fancyforms-form-entry-preview-image"><a href="' . esc_url($file) . '" target="_blank">';
                            if (in_array($file_extension, array('jpg', 'jpeg', 'png', 'gif', 'bmp'))) {
                                $upload_value .= '<img src="' . esc_url($file) . '">';
                            } else {
                                $upload_value .= '<img class="fancyforms-attachment-icon" src="' . esc_url($file_img_placeholder) . '">';
                            }
                            $upload_value .= '</a></div>';
